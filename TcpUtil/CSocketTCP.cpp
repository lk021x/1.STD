...
long CSocketTCP::Create(char* cIP,int iPort,bool bRebind)
{
	...
	{
		#ifdef _PrintError
		printf("CSocketTCP::bind failed %s \n", strerror(errno));
		#endif
		return -2;
	}
...
long CSocketTCP::Send(const char* cData, long len)
{
...
long  CSocketTCP::Listen(long lNum )
{
	...
	if((newsockfd=accept(sockfd,(struct sockaddr*)&connetAdrr, (socklen_t *)&len))<0)
	{
		#ifdef _PrintError_
		  perror("accept");
		#endif
...
long CSocketTCP::Rev(char* cData,long length)
{
	...
	ret =  recv(newsockfd,cBuffer,MAX_LEN,0);
	lenth = MAX_LEN > length ?MAX_LEN:length;
